...
@Slf4j
@Component
public class JobControlListener__2 implements JobExecutionListener {
    ...
    @Autowired
    public JobControlListener__2(ContextHeader contextHeader, ContextCache contextCache,
    ...
    private void handleJobFailed(JobExecution jobExecution) {
            ...
            for (Throwable failureException : failureExceptions) {
                if (failureException instanceof JobWithModuleIdInExecutionException) {
                    if (contextHeader.isJobHasAFile()) {
                        ...
                        errorCode = "1";
                        errorMessage = "File: " + contextHeader.getFilename() + " is already processed";
                    }
                }
            }
        }
        ...
        if (currentProcess != null) {
            ...
            ControlProcessDTO ctrlProcess = this.controlService.createControlProcessError(
                ...
                contextHeader.getModuleShdes(), jobExecution.getJobInstance().getId(), errorCode,errorMessage
                );

            if(contextHeader.isDuplicated()){
                this.controlService.createControlFile(ctrlProcess.controlProcessId(),
                    LocalDateTime.now(),
                    DateUtils.getLocalDateTimeFromString(contextHeader.getProcDate(),DateUtils.YYYYMMDD),
